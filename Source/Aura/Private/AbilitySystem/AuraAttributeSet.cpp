...
{
	// 仅仅对 Modifier 的返回值做 Clamp，当有其他的函数从 Modifier 获取数值时，返回的仍然是没有 Clamp 的原始值
...
void UAuraAttributeSet::OnRep_HealthRegeneration(const FGameplayAttributeData& OldHealthGeneration) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, HealthRegeneration, OldHealthGeneration);
}

void UAuraAttributeSet::OnRep_ManaRegeneration(const FGameplayAttributeData& OldManaGeneration) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, ManaRegeneration, OldManaGeneration);
}
